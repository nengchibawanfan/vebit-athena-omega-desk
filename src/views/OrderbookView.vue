<template>
  <div class="page">
    ...
    <PageState :loading="pageLoading" :error="pageError">
        ...
        <div class="card detail-table-card">
          <div class="card-header">
            ...
            <span class="header-actions">
              ...
              <span class="badge">买=你卖出 · 卖=你买入 · {{ orderPager.total }}笔</span>
            </span>
          </div>
          ...
          <div class="table-wrap">
            <table>
              ...
              <tbody>
                <tr
                  v-for="row in orderPager.pagedRows"
                  :key="rowId(row)"
          ...
          <TablePager
            v-model:page="orderPager.page"
            v-model:page-size="orderPager.pageSize"
            :page-count="orderPager.pageCount"
            :total="orderPager.total"
            :range-text="orderPager.rangeText"
          />
        </div>
      ...
      <div v-if="blockPage" class="page-block">
        <div class="card">
          <div class="card-header">
            <span>🐋 盘口大单</span>
            <span class="badge">真实 UID · ≥{{ appState.config.whaleThreshold }}万 · 不含做市</span>
          </div>
        ...
        <div class="kpi-grid">
          <div class="kpi-item clickable" @click="$router.push('/desk/users')">
            ...
            <div class="kpi-metrics">
              <div class="value" style="color:#6a9aff;">{{ fmtQty(blocks.kpis?.buyAmt) }}<span class="unit">万</span></div>
              <div class="qty">{{ fmtQty(blocks.kpis?.buyU) }}<span class="unit">万USDT</span></div>
            </div>
            <div class="sub">{{ blocks.kpis?.buyCount }} 笔 · 用户买 = 你卖出</div>
          </div>
          <div class="kpi-item clickable" @click="$router.push('/desk/users')">
            <div class="label">大额卖出</div>
            <div class="kpi-metrics">
              <div class="value" style="color:#ffb347;">{{ fmtQty(blocks.kpis?.sellAmt) }}<span class="unit">万</span></div>
              <div class="qty">{{ fmtQty(blocks.kpis?.sellU) }}<span class="unit">万USDT</span></div>
            </div>
            <div class="sub">{{ blocks.kpis?.sellCount }} 笔 · 用户卖 = 你买入</div>
          </div>
          <div class="kpi-item">
            ...
            <div class="sub">{{ Number(blocks.kpis?.tradeNet) >= 0 ? '用户买更多 · 你在卖出' : '用户卖更多 · 你在买入' }}</div>
          </div>
          <div class="kpi-item">
            <div class="label">最大冲击</div>
            <div class="kpi-metrics">
              <div class="value" style="color:#ff5a7a;">{{ blocks.kpis?.maxImpact }}<span class="unit">%</span></div>
            </div>
            <div class="sub">UID {{ blocks.kpis?.maxImpactUid }}</div>
          </div>
...
const {
  ...
  bindPair,
  load
} = usePageData(() => api.getBlockTrades(
...
const pageLoading = computed(() => (
  (liveLoading.value && !liveData.value) && (blockLoading.value && !blockPage.value)
))
const pageError = computed(() => {
  if (liveData.value || blockPage.value) return ''
  return liveError.value || blockError.value || ''
})

const blocks = computed(() => blockPage.value || { kpis: {}, rows: [], hours: {}, status: [] })
...
const visibleOrders = computed(() => {
  ...
  return rows.map((row) => {
    ...
    return { ...row, cumText: fmtBookQty(cum) }
  })
})
const orderPager = usePager(visibleOrders)

const book = computed(() => {
  const buckets = new Map()
  for (const row of liveData.value?.rows || []) {
  ...
  function accumulate(list, desc) {
    ...
    return sorted.map((row) => {
      ...
      return {
        ...
        amountText: fmtBookQty(row.amount),
        totalText: fmtBookQty(total)
      }
    })
  }
  ...
  return {
    bids: withPct(bids),
    asks: withPct(asks),
    bidQty: fmtBookQty(bidQty),
    askQty: fmtBookQty(askQty),
    mid,
    crossed,
    spreadLabel,
...
const cancelOption = computed(() => ({
  ...
  xAxis: { data: liveData.value?.cancelTop?.labels || [], axisLabel: { color: '#4a6080', fontSize: 9 } },
  yAxis,
  series: [{ type: 'bar', data: liveData.value?.cancelTop?.values || [], itemStyle: { color: '#ffb347' }, barWidth: '50%' }]
}))
...
const flowOption = computed(() => ({
  ...
  xAxis: {
    data: blocks.value?.hours?.labels || [],
    axisLabel: { color: '#4a6080', fontSize: 8, interval: 3 }
  },
  yAxis,
  series: [
    { name: '买入', type: 'bar', data: blocks.value?.hours?.buy || [], itemStyle: { color: '#6a9aff' }, barWidth: '28%' },
    { name: '卖出', type: 'bar', data: blocks.value?.hours?.sell || [], itemStyle: { color: '#ffb347' }, barWidth: '28%' }
  ]
}))
</script>
